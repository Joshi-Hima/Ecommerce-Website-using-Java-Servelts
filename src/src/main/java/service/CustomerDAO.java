...
public class CustomerDAO {
	...
	/*private boolean check(Customer customer) {
		...
		try {
			statement=conn.prepareStatement("select phonenumber, email from customer");
			Set= statement.executeQuery();
			
			while(Set.next())
			{	
				if(customer.getPhonenumber() == Set.getLong("phonenumber"))
				{
					isSame=true;
					break;
				}else if(customer.getEmail().equals(Set.getString("email")))
				{
					isSame=true;
					break;
				}
